...
public abstract class XmlUtils
{
            ...
            switch( event.getEventType() )
            {
            	...
            	case XMLStreamConstants.START_ELEMENT:
            	{
            		...
            		if( read && attributeValue != null )
            		{
            			...
            			for( @SuppressWarnings("unchecked") Iterator<Attribute> iterator = event.asStartElement().getAttributes(); iterator.hasNext(); )
            			{
    ...
    {
    	// for each identifying property get that property value for this document
    ...
    public static List<?> unmarshallList( final String elementName, final InputStream is, final String contextPath, final Class<?> template ) throws JAXBException, XMLStreamException, IOException
    {
    	...
    	// Original input must be prefixed and suffixed with start and end tags, as document must be valid for getElements method:
